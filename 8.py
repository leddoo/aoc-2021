inpt = """fceabd decba debgf acgefb cfedb ecf fdgaceb acfd fc gdbaec | cedbag cdeabf fdca bgadec
...
if False:
    inpt = """be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb | fdgacbe cefdb cefbgd gcbe
...
lines = [ (left.strip().split(" "), right.strip().split(" ")) for (left, right) in lines ]
...
digits_inverse = { string: digit for digit, string in digits.items() }

by_length = {}
for digit, string in digits.items():
    lst = by_length.get(len(string), [])
    lst.append(digit)
    by_length[len(string)] = lst

unique = { length: digits[0] for length, digits in by_length.items() if len(digits) == 1 }

uniques_in_right = 0
for left, right in lines:
    for string in right:
        if len(string) in unique:
            uniques_in_right += 1

#print("output unique:", uniques_in_right)


counts_by_length = {}
for length, numbers in by_length.items():
    counts = {}
    for number in numbers:
        for letter in digits[number]:
            counts[letter] = counts.get(letter, 0) + 1
    counts_by_length[length] = counts

#print("counts by length:", counts_by_length)


options_by_count_by_length = {}
for length, counts in counts_by_length.items():
    options = {}
    for letter, count in counts.items():
        if count in options:
            options[count].append(letter)
        else:
            options[count] = [letter]
    options_by_count_by_length[length] = options

#print("options by count by length:", options_by_count_by_length)
...
for left, right in lines:
    ...
    counts_by_length = {}
    for length, pattern in patterns_by_length.items():
        counts = {}
    ...
    mapping = { x: set(list("abcdefg")) for x in "abcdefg" }
    for length, counts in counts_by_length.items():
        for letter, count in counts.items():
            options = options_by_count_by_length[length][count]
            mapping[letter].intersection_update(options)
    ...
    while changed:
        ...
        for x, ys in mapping.items():
            ys = list(filter(lambda y: y not in inverse, ys))
            if len(ys) == 1:
                changed = True
                inverse[ys[0]] = x
    assert len(inverse) == 7

    mapping = { y: x for x, y in inverse.items() }
    assert len(mapping) == 7

    decoded = ""
    for number in right:
        number = digits_inverse["".join(sorted(map(lambda x: mapping[x], number)))]
        decoded += str(number)
    decoded = int(decoded)
